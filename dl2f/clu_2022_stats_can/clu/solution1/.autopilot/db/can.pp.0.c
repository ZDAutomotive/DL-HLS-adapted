...
static unsigned int internal_can_counter = 0;
...
void write_ddr(volatile unsigned char *ddr_header, char *data) {
 ...
 unsigned char ringbuffer_header_bytes[28] = {0};
  ...
  *(ddr_header + 4) = dropped_can_counter;
  ...
  memcpy(ddr_header + 28 + writeCanIndex * (12 + 80),
    data,
    (12 + 80));
...
#pragma HLS reset variable = counter_can_3
 static unsigned int counter_can_4 = 0;
#pragma HLS reset variable = counter_can_4
 static unsigned int counter_can_5 = 0;
#pragma HLS reset variable = counter_can_5
 static unsigned int counter_can_6 = 0;
#pragma HLS reset variable = counter_can_6
 static unsigned int counter_can_7 = 0;
#pragma HLS reset variable = counter_can_7
 static unsigned int counter_can_8 = 0;
#pragma HLS reset variable = counter_can_8
 static unsigned int counter_can_9 = 0;
#pragma HLS reset variable = counter_can_9
 static unsigned int counter_can_10 = 0;
#pragma HLS reset variable = counter_can_10
 static unsigned int counter_can_11 = 0;
#pragma HLS reset variable = counter_can_11
...
#pragma HLS reset variable = counter_droped
 ...
 unsigned int FrameID, orFrameID, frameDLC, DwIndex = 0, canIndex = 8;
  ...
  unsigned char can_frame[12 + 80] = {0};

  orFrameID = *(canbase + (0x2100 +(readIndex*72)) / 4);


  can_frame[12 + 4] = (orFrameID >> 19) & 0x01;
  if (((orFrameID >> 19) & 0x01) == 0) {

 FrameID = orFrameID >> 21;

 can_frame[12 + 5] = (orFrameID >> 20) & 0x01;
  } else {

 FrameID = (((orFrameID >> 21) & 0x7FF) << 18) | ((orFrameID >> 1) & 0x3FFFF);

 can_frame[12 + 5] = (orFrameID) & 0x01;
  }
  can_frame[12 + 0] = (FrameID & 0xFF000000) >> 24;
  can_frame[12 + 1] = (FrameID & 0x00FF0000) >> 16;
  can_frame[12 + 2] = (FrameID & 0x0000FF00) >> 8;
  can_frame[12 + 3] = (FrameID & 0x000000FF);




  frameDLC = *(canbase + (0x2104 +(readIndex*72)) / 4);


  unsigned int CanEDL = frameDLC;

  unsigned int Dlc = XCanFd_GetDlc2len(frameDLC & 0xF0000000, (CanEDL & 0x08000000));
  unsigned char id_can = (unsigned char)((EN + 8) & 0xFF);
  unsigned short len_can = (unsigned short)(Dlc & 0xFFFF);

  _canPacketHeader PL_Header = {
     timestamp, 2, id_can, len_can
  };
  ...
  can_frame[12 + 6] = (unsigned char) (0x00);

  can_frame[12 + 7] = (unsigned char) ((PL_Header.pkt_len_bytes) & 0xFF);

  if (CanEDL & 0x08000000) {


   VITIS_LOOP_247_1: for (int Len = 0;Len < Dlc;Len += 4) {

  int data = *(canbase + (0x2108 +(readIndex*72)
            + (DwIndex*4))/4);
  can_frame[12 + canIndex] = (data & 0xFF000000) >> 24;
  can_frame[12 + canIndex + 1] = (data & 0x00FF0000) >> 16;
  can_frame[12 + canIndex + 2] = (data & 0x0000FF00) >> 8;
  can_frame[12 + canIndex + 3] = (data & 0x000000FF);
  canIndex += 4;
  DwIndex++;
   }
  } else {
   ...
   VITIS_LOOP_261_2: for (int Len = 0;Len < Dlc;Len += 4) {
  int data = *(canbase + (0x2108 +(readIndex*72)
               + Len) / 4);
  can_frame[12 + canIndex] = (data & 0xFF000000) >> 24;
  can_frame[12 + canIndex + 1] = (data & 0x00FF0000) >> 16;
 ...
 *can_4_received = counter_can_4;
 *can_5_received = counter_can_5;
 *can_6_received = counter_can_6;
 *can_7_received = counter_can_7;
 *can_8_received = counter_can_8;
 *can_9_received = counter_can_9;
 *can_10_received = counter_can_10;
 *can_11_received = counter_can_11;

  internal_can_counter++;
  *received_can = internal_can_counter;
  write_ddr(ddr, &can_frame[0]);
  FsrVal = *(canbase + 0x0E8 / 4);
  FsrVal |= 0x00000080;
  *(canbase + 0x0E8 / 4) = FsrVal;
  return 0;
}
...
void CanFd_Recv_Sequential(volatile int *canbase,
 ...
 if (*enable_device & (0x01 << EN)) {
  ...
  int result = *(canaddr_mod + 0x0E8 / 4);
  int readIndex = 0, FifoNo = 0xFF, status = 13;
  if ((result & 0x00007F00) || (result & 0x7F000000)) {

   if (result & 0x00007F00) {
    readIndex = result & 0x0000003F;
    FifoNo = 0;
   }
   if (result & 0x7F000000) {
    readIndex = (result & 0x00800000) >> 16;
    FifoNo = 1;
   }
...
void can (volatile int *canbase,
 ...
 VITIS_LOOP_416_1: for (int jj = 0; jj < 12; jj++){
