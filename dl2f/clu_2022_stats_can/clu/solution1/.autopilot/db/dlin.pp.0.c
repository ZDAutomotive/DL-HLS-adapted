...
void single_lin_process(volatile unsigned int *linbase,
  ...
  if (PLIN_Ctrl[EN].run_state == 0) {
    ...
    VITIS_LOOP_176_1: for(int i = 0; i < (PL_Data.llen + 1); i++)
    {
...
void dlin(volatile int *linbase,
 ...
 VITIS_LOOP_256_1: for (int lin_nr = 0; lin_nr < 10; lin_nr++) dlin_FSM(linbase, ddr, enable_device, lin_nr, timestamp, received_lin);
}
