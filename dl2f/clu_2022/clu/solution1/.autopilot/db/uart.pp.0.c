...
# 5 "uart.c" 2
# 36 "uart.c"
static char init_stage[8] = {0};
static char uart_fifo[8][(12 + 192)];
...
static _uartPacketHeader PL_Header[8] = {
  {0L, 4, 0, 0}, {0L, 4, 0, 0}, {0L,4, 0, 0},
  {0L,4, 0, 0}, {0L,4, 0, 0}, {0L,4, 0, 0},
  {0L,4, 0, 0}, {0L,4, 0, 0}
};

static _uartControlStruct PL_Ctrl[8] = {
  {0L, 12, 1, 0}, {0L, 12, 1, 0}, {0L, 12, 1, 0},
  {0L, 12, 1, 0}, {0L, 12, 1, 0}, {0L, 12, 1, 0},
  {0L, 12, 1, 0}, {0L, 12, 1, 0}
};
int uart_init(volatile int *uart_baseaddr, char *debug, char EN) {
 ...
 case 0:
  *(uart_baseaddr + (0x1000 + 0x08)/4) = 0x00000001;
  *debug = *(uart_baseaddr + (0x1000 + 0x08)/4);
  break;
 case 1:
  *(uart_baseaddr + (0x1000 + 0x08)/4) = 0x00000001 | 0x00000080;
  *debug = *(uart_baseaddr + (0x1000 + 0x08)/4);
  break;
 case 2:
  lcrReg = *(uart_baseaddr + (0x1000 + 0x0C)/4);
  *(uart_baseaddr + (0x1000 + 0x0C)/4) = lcrReg | 0x00000080;
  *debug = *(uart_baseaddr + (0x1000 + 0x0C)/4);
  break;
 case 3:
  *(uart_baseaddr + (0x1000 + 0x00)/4) = 0xFF;
  *debug = *(uart_baseaddr + (0x1000 + 0x0C)/4);
  break;
 case 4:
  *(uart_baseaddr + (0x1000 + 0x04)/4) = BaudMSB;
  *debug = *(uart_baseaddr + (0x1000 + 0x04)/4);
  break;
 case 5:
  *(uart_baseaddr + (0x1000 + 0x00)/4) = BaudLSB;
  *debug = *(uart_baseaddr + (0x1000 + 0x00)/4);
  break;
 case 6:
  *(uart_baseaddr + (0x1000 + 0x0C)/4) = lcrReg;
  *debug = *(uart_baseaddr + (0x1000 + 0x0C)/4);
  break;
 case 7:
  lcrReg = *(uart_baseaddr + (0x1000 + 0x0C)/4);
...
void ddr_write(volatile unsigned char *ddr_header, char device_id) {
  ...
  *(ddr_header + 4) = dropped_uart_counter;
  ...
  memcpy(ddr_header + 28 + writeUartIndex * (12 + 192),
      uart_fifo[device_id],
      (12 + 192));
  update_write_index(ddr_header, nextWriteUartIndex);
 }

 PL_Ctrl[device_id].fifo_index = 12;
...
void uart_data_read (volatile unsigned char* ddr,
 ...
 if (*init_device & enable) {
  ...
  if((LSRREG & 0x10) || (LSRREG & 0x1)) {
   ...
   char rxData = (char) *(uartbase_delta + 0x1000);
  ...
  if ((PL_Ctrl[device_id].fifo_index == (12 + 192)) || (time_delta > 1000000)) {
...
void uart(volatile int *uartbase,
 ...
 VITIS_LOOP_240_1: for (int uart_i = 0; uart_i < 8; uart_i++) uart_data_read (ddr, uartbase, ts_in, init_device, uart_i, received_uart);
}
