...
int recvFrame_logic (volatile int *canbase,
 ...
 can_frame[12 + 5] = (orFrameID) & 0x01;
  ...
  if (CanEDL & 0x08000000) {
   ...
   VITIS_LOOP_206_1: for (int Len = 0;Len < Dlc;Len += 4) {
#pragma HLS PIPELINE II=3
 ...
 int data = *(canbase + (0x2108 +(readIndex*72)
  ...
  DwIndex++;
   ...
   VITIS_LOOP_221_2: for (int Len = 0;Len < Dlc;Len += 4) {
#pragma HLS PIPELINE II=3
 int data = *(canbase + (0x2108 +(readIndex*72)
               + Len) / 4);
  can_frame[12 + canIndex] = (data & 0xFF000000) >> 24;
...
void CanFd_Recv_Sequential(volatile int *canbase,
 ...
 if (*enable_device & (0x01 << EN)) {
  ...
  if ((result & 0x00007F00) || (result & 0x7F000000)) {
   ...
   status = recvFrame_logic(canaddr_mod, ddr, FifoNo, readIndex, result, EN, timestamp, received_can);
  }
 }
}
...
void can (volatile int *canbase,
 ...
 VITIS_LOOP_277_1: for (int jj = 0; jj < 12; jj++) CanFd_Recv_Sequential(canbase, ddr, enable, jj, timestamp, received_can);
}
