...
port (
    ...
    can_ptr               :out  STD_LOGIC_VECTOR(63 downto 0);
    uart_ptr              :out  STD_LOGIC_VECTOR(63 downto 0);
    lin_ptr               :out  STD_LOGIC_VECTOR(63 downto 0);
    ...
    can_ddr               :out  STD_LOGIC_VECTOR(63 downto 0);
    uart_ddr              :out  STD_LOGIC_VECTOR(63 downto 0);
    lin_ddr               :out  STD_LOGIC_VECTOR(63 downto 0);
...
architecture behave of clu_EN_s_axi is
    ...
    constant ADDR_CAN_PTR_DATA_1       : INTEGER := 16#14#;
    constant ADDR_CAN_PTR_CTRL         : INTEGER := 16#18#;
    constant ADDR_UART_PTR_DATA_0      : INTEGER := 16#1c#;
    constant ADDR_UART_PTR_DATA_1      : INTEGER := 16#20#;
    constant ADDR_UART_PTR_CTRL        : INTEGER := 16#24#;
    constant ADDR_LIN_PTR_DATA_0       : INTEGER := 16#28#;
    constant ADDR_LIN_PTR_DATA_1       : INTEGER := 16#2c#;
    constant ADDR_LIN_PTR_CTRL         : INTEGER := 16#30#;
    constant ADDR_RECEIVED_CAN_DATA_0  : INTEGER := 16#34#;
    constant ADDR_RECEIVED_CAN_CTRL    : INTEGER := 16#38#;
    constant ADDR_RECEIVED_UART_DATA_0 : INTEGER := 16#44#;
    constant ADDR_RECEIVED_UART_CTRL   : INTEGER := 16#48#;
    constant ADDR_RECEIVED_LIN_DATA_0  : INTEGER := 16#54#;
    constant ADDR_RECEIVED_LIN_CTRL    : INTEGER := 16#58#;
    constant ADDR_CAN_EN_DATA_0        : INTEGER := 16#64#;
    constant ADDR_CAN_EN_CTRL          : INTEGER := 16#68#;
    constant ADDR_UART_EN_DATA_0       : INTEGER := 16#6c#;
    constant ADDR_UART_EN_CTRL         : INTEGER := 16#70#;
    constant ADDR_LIN_EN_DATA_0        : INTEGER := 16#74#;
    constant ADDR_LIN_EN_CTRL          : INTEGER := 16#78#;
    constant ADDR_CAN_DDR_DATA_0       : INTEGER := 16#7c#;
    constant ADDR_CAN_DDR_DATA_1       : INTEGER := 16#80#;
    constant ADDR_CAN_DDR_CTRL         : INTEGER := 16#84#;
    constant ADDR_UART_DDR_DATA_0      : INTEGER := 16#88#;
    constant ADDR_UART_DDR_DATA_1      : INTEGER := 16#8c#;
    constant ADDR_UART_DDR_CTRL        : INTEGER := 16#90#;
    constant ADDR_LIN_DDR_DATA_0       : INTEGER := 16#94#;
    constant ADDR_LIN_DDR_DATA_1       : INTEGER := 16#98#;
    constant ADDR_LIN_DDR_CTRL         : INTEGER := 16#9c#;
    ...
    signal int_can_ptr         : UNSIGNED(63 downto 0) := (others => '0');
    signal int_uart_ptr        : UNSIGNED(63 downto 0) := (others => '0');
    signal int_lin_ptr         : UNSIGNED(63 downto 0) := (others => '0');
    ...
    signal int_can_ddr         : UNSIGNED(63 downto 0) := (others => '0');
    signal int_uart_ddr        : UNSIGNED(63 downto 0) := (others => '0');
    signal int_lin_ddr         : UNSIGNED(63 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_CAN_PTR_DATA_1 =>
                        rdata_data <= RESIZE(int_can_ptr(63 downto 32), 32);
                    when ADDR_UART_PTR_DATA_0 =>
                        rdata_data <= RESIZE(int_uart_ptr(31 downto 0), 32);
                    when ADDR_UART_PTR_DATA_1 =>
                        rdata_data <= RESIZE(int_uart_ptr(63 downto 32), 32);
                    when ADDR_LIN_PTR_DATA_0 =>
                        rdata_data <= RESIZE(int_lin_ptr(31 downto 0), 32);
                    when ADDR_LIN_PTR_DATA_1 =>
                        rdata_data <= RESIZE(int_lin_ptr(63 downto 32), 32);
                    ...
                    when ADDR_CAN_DDR_DATA_1 =>
                        rdata_data <= RESIZE(int_can_ddr(63 downto 32), 32);
                    when ADDR_UART_DDR_DATA_0 =>
                        rdata_data <= RESIZE(int_uart_ddr(31 downto 0), 32);
                    when ADDR_UART_DDR_DATA_1 =>
                        rdata_data <= RESIZE(int_uart_ddr(63 downto 32), 32);
                    when ADDR_LIN_DDR_DATA_0 =>
                        rdata_data <= RESIZE(int_lin_ddr(31 downto 0), 32);
                    when ADDR_LIN_DDR_DATA_1 =>
                        rdata_data <= RESIZE(int_lin_ddr(63 downto 32), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_CAN_PTR_DATA_1) then
                    int_can_ptr(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_can_ptr(63 downto 32));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_UART_PTR_DATA_1) then
                    int_uart_ptr(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_uart_ptr(63 downto 32));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_LIN_PTR_DATA_1) then
                    int_lin_ptr(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_lin_ptr(63 downto 32));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_CAN_DDR_DATA_1) then
                    int_can_ddr(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_can_ddr(63 downto 32));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_UART_DDR_DATA_1) then
                    int_uart_ddr(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_uart_ddr(63 downto 32));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_LIN_DDR_DATA_1) then
                    int_lin_ddr(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_lin_ddr(63 downto 32));
                end if;
            end if;
        end if;
    end process;
