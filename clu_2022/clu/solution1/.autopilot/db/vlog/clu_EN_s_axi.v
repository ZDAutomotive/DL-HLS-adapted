...
#(parameter
    ...
    output wire [63:0]                   can_ptr,
    output wire [63:0]                   uart_ptr,
    output wire [63:0]                   lin_ptr,
    ...
    output wire [63:0]                   can_ddr,
    output wire [63:0]                   uart_ddr,
    output wire [63:0]                   lin_ddr,
...
localparam
    ...
    ADDR_CAN_PTR_DATA_1       = 8'h14,
    ADDR_CAN_PTR_CTRL         = 8'h18,
    ADDR_UART_PTR_DATA_0      = 8'h1c,
    ADDR_UART_PTR_DATA_1      = 8'h20,
    ADDR_UART_PTR_CTRL        = 8'h24,
    ADDR_LIN_PTR_DATA_0       = 8'h28,
    ADDR_LIN_PTR_DATA_1       = 8'h2c,
    ADDR_LIN_PTR_CTRL         = 8'h30,
    ADDR_RECEIVED_CAN_DATA_0  = 8'h34,
    ADDR_RECEIVED_CAN_CTRL    = 8'h38,
    ADDR_RECEIVED_UART_DATA_0 = 8'h44,
    ADDR_RECEIVED_UART_CTRL   = 8'h48,
    ADDR_RECEIVED_LIN_DATA_0  = 8'h54,
    ADDR_RECEIVED_LIN_CTRL    = 8'h58,
    ADDR_CAN_EN_DATA_0        = 8'h64,
    ADDR_CAN_EN_CTRL          = 8'h68,
    ADDR_UART_EN_DATA_0       = 8'h6c,
    ADDR_UART_EN_CTRL         = 8'h70,
    ADDR_LIN_EN_DATA_0        = 8'h74,
    ADDR_LIN_EN_CTRL          = 8'h78,
    ADDR_CAN_DDR_DATA_0       = 8'h7c,
    ADDR_CAN_DDR_DATA_1       = 8'h80,
    ADDR_CAN_DDR_CTRL         = 8'h84,
    ADDR_UART_DDR_DATA_0      = 8'h88,
    ADDR_UART_DDR_DATA_1      = 8'h8c,
    ADDR_UART_DDR_CTRL        = 8'h90,
    ADDR_LIN_DDR_DATA_0       = 8'h94,
    ADDR_LIN_DDR_DATA_1       = 8'h98,
    ADDR_LIN_DDR_CTRL         = 8'h9c,
    WRIDLE                    = 2'd0,
    WRDATA                    = 2'd1,
    WRRESP                    = 2'd2,
    WRRESET                   = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_can_ptr = 'b0;
    reg  [63:0]                   int_uart_ptr = 'b0;
    reg  [63:0]                   int_lin_ptr = 'b0;
    ...
    reg  [63:0]                   int_can_ddr = 'b0;
    reg  [63:0]                   int_uart_ddr = 'b0;
    reg  [63:0]                   int_lin_ddr = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_CAN_PTR_DATA_1: begin
                    rdata <= int_can_ptr[63:32];
                end
                ADDR_UART_PTR_DATA_0: begin
                    rdata <= int_uart_ptr[31:0];
                end
                ADDR_UART_PTR_DATA_1: begin
                    rdata <= int_uart_ptr[63:32];
                end
                ADDR_LIN_PTR_DATA_0: begin
                    rdata <= int_lin_ptr[31:0];
                end
                ADDR_LIN_PTR_DATA_1: begin
                    rdata <= int_lin_ptr[63:32];
                end
                ...
                ADDR_CAN_DDR_DATA_1: begin
                    rdata <= int_can_ddr[63:32];
                end
                ADDR_UART_DDR_DATA_0: begin
                    rdata <= int_uart_ddr[31:0];
                end
                ADDR_UART_DDR_DATA_1: begin
                    rdata <= int_uart_ddr[63:32];
                end
                ADDR_LIN_DDR_DATA_0: begin
                    rdata <= int_lin_ddr[31:0];
                end
                ADDR_LIN_DDR_DATA_1: begin
                    rdata <= int_lin_ddr[63:32];
                end
            endcase
        end
    end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_CAN_PTR_DATA_1)
            int_can_ptr[63:32] <= (WDATA[31:0] & wmask) | (int_can_ptr[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_uart_ptr[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_UART_PTR_DATA_1)
            int_uart_ptr[63:32] <= (WDATA[31:0] & wmask) | (int_uart_ptr[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_LIN_PTR_DATA_1)
            int_lin_ptr[63:32] <= (WDATA[31:0] & wmask) | (int_lin_ptr[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_CAN_DDR_DATA_1)
            int_can_ddr[63:32] <= (WDATA[31:0] & wmask) | (int_can_ddr[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_uart_ddr[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_UART_DDR_DATA_1)
            int_uart_ddr[63:32] <= (WDATA[31:0] & wmask) | (int_uart_ddr[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_LIN_DDR_DATA_1)
            int_lin_ddr[63:32] <= (WDATA[31:0] & wmask) | (int_lin_ddr[63:32] & ~wmask);
    end
end
